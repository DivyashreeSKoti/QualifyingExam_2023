...
# Track the original number of batches
...
# ******** Section to get validation probabilities, used later for generalization calculation
true_labels = []
predicted_probs = []
predicted_labels = []
for batch_data, batch_targets in val_dataloader:
...
def write_to_csv(data):
    ...
    temp_folder_path = fp + '/probability_output_files/Weak_Generalization/'+ file_str
    ...
    output_file = temp_folder_path +'/removeoutput_file_'+ str(os.getpid()) + '_' + file_str

    df = pd.DataFrame(data)
    
    df.to_csv(output_file+'.csv', index=False)
    torch.save(pytorch_model, output_file+'.pth')
