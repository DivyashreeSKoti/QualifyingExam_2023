...
dataset, targets, _ = DL.load_dataset(folder_path)
...
train_subsampled_dataloader = DL.CustomDataLoader(train_data, train_targets, batch_size=batch_size, num_batches=100, subsample_size=subsample_size, shuffle=True)
...
for epoch in range(num_epochs):
    ...
    for i, (batch_data_1,batch_data_2) in enumerate(val_subsampled_dataloader):
        ...
        batch_data_1 = batch_data_1.to(device)
        batch_data_2 = batch_data_2.to(device)
        val_loss, contrastive_acc = cm.train_step((batch_data_1, batch_data_2))
...
masked_encoder_model = fp+'/saved_models'+'/masked_encoder_'+str(embed_dim)+'_'+str(num_heads)+'_'+str(num_induce)+'_'+str(stack)+'_'+str(dropout)+'.pth'
unmasked_encoder_model = fp+'/saved_models'+'/unmasked_encoder_'+str(embed_dim)+'_'+str(num_heads)+'_'+str(num_induce)+'_'+str(stack)+'_'+str(dropout)+'.pth'
